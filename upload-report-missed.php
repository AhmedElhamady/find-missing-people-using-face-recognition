...
if (!isset($_SESSION["id"])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        } elseif (!(isset($_POST['relevance']) && !empty($_POST['relevance']))) {
        ...
        if (isset($_SESSION["report-errors"])) {
            ...
            // $destination2 = $directory2 . $filename;
            // copy($dst_fname, $dir);
            // move_uploaded_file($_FILES["photo"]["tmp_name"], $dir);
            // $dst_fname = "model/" . $photo_name;
            // $dst_fname = str_replace('\\', '/', $dst_fname);
            // move_uploaded_file($photo_tmp_name, $dst_fname);
            ...
            if ($resultObj->save == "noface") {
                ...
                $relevance = mysqli_escape_string($connection, $_POST["relevance"]);
